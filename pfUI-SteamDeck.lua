...
importBtn:SetScript("OnClick", function()
    -- We try to use the pfUI namespace. If it's not ready, we warn the user.
    if _G["pfUI"] and pfUI.api and pfUI.api.Base64Decode then
        ...
        if func then
            ...
            DEFAULT_CHAT_FRAME:AddMessage("|cff33ff99pfUI-SD:|r Error: Profile data could not be parsed.")
        end
    else
        DEFAULT_CHAT_FRAME:AddMessage("|cff00ffffpfUI-SD:|r pfUI is not ready. Try waiting 5 seconds or reloading.")
    end
end)
...
-- 5. REFRESH LOOP & EVENT HANDLER
local f = CreateFrame("Frame")
f:RegisterEvent("PLAYER_LOGIN") -- This triggers when the game world finishes loading
f:SetScript("OnEvent", function()
    if event == "PLAYER_LOGIN" then
        DEFAULT_CHAT_FRAME:AddMessage("|cff33ff99pfUI-SD:|r Ready. Type /pfsd for settings.")
    end
end)
